...
/** Original layered art, generated by the `sample` script. */
...
async function start(): Promise<void> {
  try {
    ...
    if (server === null) await sample(editor).catch(report);
 ...
 */
async function sample(editor: Editor): Promise<Session> {
  ...
  return editor.openFile(new Uint8Array(await response.arrayBuffer()), "Mica.clm");
}
...
function mount(editor: Editor): void {
  ...
  createRoot(host).render(
    <StrictMode>
      <CatchlightEditor editor={editor} onOpenStarter={() => sample(editor)} />
    </StrictMode>,
  );
}
...
function probe(editor: Editor): void {
  ...
  (globalThis as unknown as Record<string, unknown>).__catchlightProbe = {
    editor,
    fitCamera,
  };
}
...
function describe(cause: unknown): string {
  if (cause instanceof Error) {
    return cause.cause === undefined ? cause.message : `${cause.message}: ${describe(cause.cause)}`;
  }
  return String(cause);
}
