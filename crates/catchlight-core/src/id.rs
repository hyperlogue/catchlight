...
impl NodeId {
    /// Wraps a string this crate just built to the charset — the root's Id,
    /// which is a constant rather than a draw. [`generated`] debug-asserts the
    /// charset, so a minter that drifts off it fails a test rather than
    /// writing a file.
