...
fn mip_checker() -> (LegacyDocument, Vec<LegacyTexture>) {
    ...
    (doc, vec![checker_texture(CHECKER_TEXELS)])
}
...
fn composite_blit_uniforms() -> (LegacyDocument, Vec<LegacyTexture>) {
    ...
    let mut nodes = vec![root, backdrop];
    for c in BLIT_CELLS {
        let cell = nodes.len() as u32;
        nodes.push(composite_node(
            c.name,
            c.x,
            c.opacity,
            c.tint,
        ...
        nodes.push(cell_quad(cell, "back", 1, 0.0));
        nodes.push(cell_quad(cell, "front", 2, CELL_FRONT_OFFSET));
    }
...
mod tests {
    ...
    /// table (comparing encoded PNG bytes would break on any encoder change),
    /// and the whole point of this fixture is the texture: minifying a
